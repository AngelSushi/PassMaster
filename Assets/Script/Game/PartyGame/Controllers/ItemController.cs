...
public class ItemAction : MonoBehaviour {
    ...
    public bool CheckAction(GameObject targetPlayer) {
        switch(type) {
    ...
    public bool CheckInRangeAction(GameObject targetPlayer) { 
        ...
        // il peut avoir plusieurs step a x range du joueur 
        stepsActionPath.Clear();

        bool sameParent = true;
        if(beginStep.transform.parent != end.transform.parent) {
            end = beginStep.transform.parent.GetChild(beginStep.transform.parent.childCount - 1).gameObject;
            sameParent = false;
        }

        
        return FindSmallestPathTo(beginStep,,rangeType,stepsActionPath,false,sameParent); 
